...
def build_engine_onnx(model_file, verbose=False):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(flags=network_flags) as network, trt.OnnxParser(network, TRT_LOGGER) as parser:
        ...
        config.add_optimization_profile(build_profile(builder, network, profile_shapes={'input' : [(100, 1, 224, 224),(100, 1, 224, 224),(100, 1, 224, 224)]}))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--onnx", type=str, default='lenet.onnx', help="Path to RN50 ONNX graph")
    parser.add_argument("--engine", type=str,  default='lenet_trt.engine', help="output path to TensorRT engine")
